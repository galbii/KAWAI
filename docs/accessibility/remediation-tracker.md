...
| D7 | Heading hierarchy skips levels (an `<h3>` with no preceding `<h2>`): a hero display heading, and the dealer card title. | 1.3.1 Info & Relationships (A) | Moderate | Hero component (open); `find-a-dealer/components/DealerCard.tsx` (deferred) | **Open** — DealerCard is in an unrelated in-progress branch of work; apply after that lands. Hero `<h3>` pending. | — |
...
| D9 | Dealer type badges (Shigeru/Acoustic/Digital tags) low contrast. | 1.4.3 (AA) | Moderate | `find-a-dealer/components/DealerCard.tsx` | **Open — blocked** by unrelated in-progress work in that file; fix when it lands. | — |
