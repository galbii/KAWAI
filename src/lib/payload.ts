...
// Fetch LandingPage data from API
export async function getLandingPage(dealerSlug: string, campaignSlug: string): Promise<any | null> {
  try {
    // Construct absolute URL for server-side requests
    let apiUrl = `/api/landing-pages/by-slug/${dealerSlug}/${campaignSlug}`
    if (typeof window === 'undefined') {
      // Server-side: need absolute URL
      const baseURL = process.env.NEXT_PUBLIC_SITE_URL || 'http://localhost:3000'
      apiUrl = `${baseURL}/api/landing-pages/by-slug/${dealerSlug}/${campaignSlug}`
    }
    
    console.log('[DEBUG] Fetching landing page data from', apiUrl)
    ...
    if (response.status === 404) {
      // Landing page not found or inactive
      console.log('[DEBUG] Landing page not found or inactive')
      return null
    }
    ...
    if (!result.success) {
      if (result.error === 'Landing page not found or inactive') {
        return null
      }
      throw new Error(result.error || 'Failed to fetch landing page data')
    }
    
    console.log('[DEBUG] Successfully fetched landing page data')
    return result.data
  } catch (error) {
    // Handle null/undefined errors the same way as other payloadFetch functions
    if (error === null || error === undefined) {
      console.error('[ERROR] Failed to fetch landing page: received null error')
      throw new Error('Failed to fetch landing page data')
    }
    
    if (!(error instanceof Error)) {
      console.error('[ERROR] Failed to fetch landing page:', String(error))
      throw new Error(`Failed to fetch landing page data: ${String(error)}`)
    }
    
    console.error('[ERROR] Failed to fetch landing page:', error.message)
    throw error
  }
}

// Cached version of LandingPage API function
export async function getCachedLandingPage(dealerSlug: string, campaignSlug: string): Promise<any | null> {
  const cacheKey = `landing-page-${dealerSlug}-${campaignSlug}`
  const cached = getCachedData<any | null>(cacheKey)
  
  if (cached !== null) return cached
  
  const data = await getLandingPage(dealerSlug, campaignSlug)
  setCachedData(cacheKey, data)
  
  return data
}

// Get complete LandingPage data with validation and error handling
export async function getLandingPageData(dealerSlug: string, campaignSlug: string): Promise<{
  landingPage: any
  dealerLocation: any
  pageContent: any[]
  seo: any
  ...
  try {
    ...
    // Query by campaign slug and dealer relationship
    queryParams.append('where[slug][equals]', campaignSlug)
    queryParams.append('where[dealerLocation.slug][equals]', dealerSlug)
    // Support both new status field and legacy isActive field for backward compatibility
    queryParams.append('where[or][0][status][equals]', 'active')
    queryParams.append('where[or][1][isActive][equals]', 'true')
    queryParams.append('limit', '1')
    queryParams.append('depth', '3') // Populate dealerLocation and all nested relationships
    
    const endpoint = `/landing-pages?${queryParams.toString()}`
    const response = await payloadFetch<LandingPagesResponse>(endpoint)
    
    console.log(`[DEBUG] getLandingPageData: Found ${response.docs.length} landing pages for dealer="${dealerSlug}", campaign="${campaignSlug}"`)
    
    const landingPageData = response.docs[0]
    if (!landingPageData) {
      return null
    }
    
    // Check if campaign is within active date range (if dates are set)
    const now = new Date()
    
    if ((landingPageData as any).campaignDates?.startDate) {
      const startDate = new Date((landingPageData as any).campaignDates.startDate)
      if (now < startDate) {
        console.log('[DEBUG] Campaign has not started yet')
        return null // Campaign hasn't started
      }
    }
    
    if ((landingPageData as any).campaignDates?.endDate) {
      const endDate = new Date((landingPageData as any).campaignDates.endDate)
      if (now > endDate) {
        console.log('[DEBUG] Campaign has expired')
    ...
    return {
      ...
      dealerLocation: landingPageData.dealerLocation, // Populated relationship
      pageContent: landingPageData.pageContent || [],
      seo: landingPageData.seo
    }
  } catch (error) {
    // Handle null/undefined errors the same way as other functions
    if (error === null || error === undefined) {
      console.error('[ERROR] Failed to fetch landing page data: received null error')
      return null
    }
    
    if (!(error instanceof Error)) {
      console.error('[ERROR] Failed to fetch landing page data:', String(error))
      return null
    }
    
    console.error('[ERROR] Failed to fetch landing page data:', error.message)
    return null
  }
}
...
export async function getActiveLandingPages(dealerSlug?: string): Promise<LandingPage[]> {
  try {
    ...
    // Optionally filter by dealer location
    if (dealerSlug) {
      queryParams.append('where[dealerLocation.slug][equals]', dealerSlug)
    }
    ...
    queryParams.append('limit', '100') // Get all active landing pages
    queryParams.append('depth', '2') // Populate dealerLocation relationship and media
    
    const endpoint = `/landing-pages?${queryParams.toString()}`
    const response = await payloadFetch<LandingPagesResponse>(endpoint)
    
    console.log(`[DEBUG] getActiveLandingPages: Found ${response.docs.length} active landing pages${dealerSlug ? ` for dealer="${dealerSlug}"` : ''}`)
    
    return response.docs
  } catch (error) {
    // Handle errors the same way as other functions
    if (error === null || error === undefined) {
      console.error('[ERROR] Failed to fetch active landing pages: received null error')
      return []
    }
    
    if (!(error instanceof Error)) {
      console.error('[ERROR] Failed to fetch active landing pages:', String(error))
      return []
    }
    
    console.error('[ERROR] Failed to fetch active landing pages:', error.message)
    return []
  }
}

// Fetch SEO metadata for a landing page by dealer slug and campaign slug
export async function getLandingPageMetadata(dealerSlug: string, campaignSlug: string): Promise<{
  title: string
  description: string
  keywords?: string
  metaTitle?: string
  ...
  try {
    ...
    queryParams.append('where[or][1][isActive][equals]', 'true')
    queryParams.append('limit', '1')
    queryParams.append('depth', '2') // Populate media relationships in SEO fields
    
    const endpoint = `/landing-pages?${queryParams.toString()}`
    const response = await payloadFetch<LandingPagesResponse>(endpoint)
    
    const landingPage = response.docs[0]
    
    if (!landingPage) {
      console.log(`[DEBUG] getLandingPageMetadata: No landing page found for dealer="${dealerSlug}", campaign="${campaignSlug}"`)
      return null
    }
    
    console.log(`[DEBUG] getLandingPageMetadata: Found metadata for landing page "${(landingPage as any).campaignId}"`)
    
    // Return structured metadata following SEO best practices
    return {
      title: (landingPage as any).title,
      description: (landingPage as any).description,
      keywords: (landingPage as any).seo?.keywords,
      metaTitle: (landingPage as any).seo?.metaTitle || (landingPage as any).title,
      metaDescription: (landingPage as any).seo?.metaDescription || (landingPage as any).description,
      openGraphTitle: (landingPage as any).seo?.openGraphTitle || (landingPage as any).seo?.metaTitle || (landingPage as any).title,
      openGraphDescription: (landingPage as any).seo?.openGraphDescription || (landingPage as any).seo?.metaDescription || (landingPage as any).description,
      openGraphImage: (landingPage as any).seo?.openGraphImage,
      canonicalUrl: (landingPage as any).seo?.canonicalUrl,
      robotsIndex: (landingPage as any).seo?.robotsIndex ?? true // Default to true if not specified
    }
  } catch (error) {
    // Handle errors the same way as other functions
    if (error === null || error === undefined) {
      console.error('[ERROR] Failed to fetch landing page metadata: received null error')
      return null
    }
    
    if (!(error instanceof Error)) {
      console.error('[ERROR] Failed to fetch landing page metadata:', String(error))
      return null
    }
    
    console.error('[ERROR] Failed to fetch landing page metadata:', error.message)
    return null
  }
}

// Cached versions of Landing Page API functions
export async function getCachedLandingPageData(dealerSlug: string, campaignSlug: string): Promise<{
  landingPage: any
  dealerLocation: any
  pageContent: any[]
  seo: any
} | null> {
  const cacheKey = `landing-page-data-${dealerSlug}-${campaignSlug}`
  ...
  const data = await getLandingPageData(dealerSlug, campaignSlug)
  setCachedData(cacheKey, data)
...
export async function getCachedActiveLandingPages(dealerSlug?: string): Promise<LandingPage[]> {
  ...
  const data = await getActiveLandingPages(dealerSlug)
  setCachedData(cacheKey, data)
  
  return data
}

export async function getCachedLandingPageMetadata(dealerSlug: string, campaignSlug: string): Promise<ReturnType<typeof getLandingPageMetadata>> {
  const cacheKey = `landing-page-metadata-${dealerSlug}-${campaignSlug}`
  const cached = getCachedData<ReturnType<typeof getLandingPageMetadata>>(cacheKey)
  
  if (cached !== null) return cached
  
  const data = await getLandingPageMetadata(dealerSlug, campaignSlug)
  setCachedData(cacheKey, data)
