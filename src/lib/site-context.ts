...
export function getSiteAlternates(path: string) {
  return {
    ...
    // No fr-CA entry: French is produced client-side by the visitor's browser,
    // so there is no French URL to advertise. Adding one would point crawlers
    // at a page that serves English.
