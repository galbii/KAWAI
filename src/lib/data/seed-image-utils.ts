...
export async function uploadImageToMedia(
  ...
  try {
    ...
    if (!fs.existsSync(absolutePath)) {
      payload.logger.warn(`Warning: Image not found: ${imagePath}`)
      return null
    }
...
export async function uploadAllPianoPageImages(payload: Payload): Promise<{
  ...
  payload.logger.info('Starting piano page image uploads...')
...
export async function getOrCreateFallbackImage(payload: Payload): Promise<string | null> {
  try {
    ...
    payload.logger.warn('No fallback image found, proceeding without fallback')
    return null
  } catch (error) {
