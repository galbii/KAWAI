...
export function filterPianosBySpecs(pianos: any[], filters: any): any[] {
  return pianos.filter(piano => {
    ...
    if (filters.finishes && filters.finishes.length > 0) {
      ...
      if (!filters.finishes.some((finish: any) => pianoFinishes.includes(finish))) return false
    }
...
export function generatePianoSearchIndex(pianos: any[]): Array<{
  id: string
  searchTerms: string
}> {
  return pianos.map(piano => ({
