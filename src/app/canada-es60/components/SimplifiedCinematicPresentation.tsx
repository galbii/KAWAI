...
import { SLIDE_COMPONENTS, SLIDE_NAMES } from './slides/index';
...
export function SimplifiedCinematicPresentation() {
  ...
  // Show promotional popup after opening sequence finishes (7 seconds after mount on first slide)
  useEffect(() => {
    // Only trigger if we're on the first slide
    if (currentSlide !== 0) return;

    // Delay popup by 7 seconds to allow all opening animations to complete
    // Timeline: ES60 logo (2s) + Button animation finishes (4.8s) = ~7s total
    //
    // IMPORTANT: This auto-opens the popup WITHOUT triggering analytics tracking.
    // Tracking only occurs when users manually click "ES60 Bundle" buttons.
  ...
  // Callback to open the promotional popup from slide buttons
  // NOTE: This function only opens the popup. Analytics tracking happens
  // in each slide's handleBundleClick() function BEFORE calling this callback.
  // This ensures tracking only fires for manual user interactions, not auto-opens.
    ...
    <div className="relative">
      ...
      >
        {SLIDE_COMPONENTS.map((SlideComponent, index) => (
          ...
          >
            <SlideComponent onOpenBundle={handleOpenBundle} />
          </div>
        ))}
      </div>
      ...
      {/* Promotional Popup - Canada Exclusive Bundle */}
      <PromotionalPopup
        show={showPromotionalPopup}
        onClose={() => setShowPromotionalPopup(false)}
      />
    </div>
  );
}
