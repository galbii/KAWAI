 ...
 *
 * Currently OFF, matching /signup2: the extra step is disabled and the lead
 * routes purely on the ZIP they already entered — `notifyRsmOfLead` geocodes it
 * and matches the nearest RSM-managed dealer, exactly as /signup does. Flip this
 * back to `true` to restore the picker; the whole switch lives in this one
 * constant and everything it needs (PostSignupDealerPicker,
 * captureLead/confirmLead) is still wired up below.
 */
...
export function OfferModalProvider({
  ...
  const captureLead = useCallback((data: PreFormValues) => {
    ...
    // Deliberately not gated on `testMode`: routing is exactly what test mode
    // exists to exercise, and the LEAD_NOTIFY_* switches hold or redirect it.
