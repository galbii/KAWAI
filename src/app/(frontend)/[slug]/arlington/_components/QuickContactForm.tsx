...
export default function QuickContactForm({ onSuccess, onSkip }: QuickContactFormProps) {
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      // Submit to API
      console.log('QuickContactForm: Submitting to API...', formData);

      const response = await fetch('/api/arlington/booking', {
        method: 'POST',
        headers: {
      ...
      console.log('QuickContactForm: Successfully submitted to API', data);
      ...
      console.error('QuickContactForm: Submission error:', error);
      setSubmitError(
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        {onSkip && (
          <button
            type="button"
            onClick={onSkip}
            className="w-full text-kawai-black/60 hover:text-kawai-black text-sm font-medium transition-colors"
            disabled={isSubmitting}
          >
