...
/**
 * Storefront picker for campaign CTAs that live outside a storefront context
 * (homepage carousel slides, etc.) — the visitor chooses a location and is
 * routed to that store's campaign page via `hrefForSlug`.
 *
 * Generalized from the Grand Spring Sale location modal so each campaign
 * doesn't fork its own copy; SpringSaleLocationModal now wraps this.
 */
...
interface Props {
  isOpen: boolean
  onClose: () => void
  /** Builds the destination for a chosen store, e.g. slug => `/store/${slug}/back-to-school`. */
  hrefForSlug: (slug: string) => string
}

export function CampaignLocationModal({ isOpen, onClose, hrefForSlug }: Props) {
