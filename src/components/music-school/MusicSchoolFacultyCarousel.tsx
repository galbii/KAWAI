...
export function MusicSchoolFacultyCarousel({ faculty, schoolName, about, baseUrl }: Props) {
  ...
  return (
    <div className="grid md:grid-cols-[5fr_6fr] min-h-[600px]">
      ...
      {/* Photo panel — cross-fade between members */}
      <div className="relative bg-kawai-black overflow-hidden min-h-[400px] md:min-h-0">

        {/* Outgoing photo */}
        {prevIndex !== null && faculty[prevIndex]?.photoUrl && (
          <Image
            key={`prev-${prevIndex}`}
            src={faculty[prevIndex]!.photoUrl!}
            alt=""
            fill
      ...
      <div className="flex flex-col justify-between px-10 py-12 md:px-14 lg:px-20 bg-white">
        ...
        {about && (
          <div className="mb-8">
            <p className="text-[10px] font-bold tracking-[0.2em] uppercase text-kawai-charcoal/30 mb-2">
              About
            </p>
            <p className="text-kawai-charcoal/55 text-sm leading-relaxed max-w-sm">
        ...
        {/* Faculty detail — slides up on change */}
        <div
          key={index}
          className="flex-1"
          style={{
            animation: 'faculty-enter 0.7s ease forwards',
          }}
        >
          <style>{`
          ...
          <p className="text-[10px] font-bold tracking-[0.22em] uppercase text-kawai-charcoal/30 mb-5">
            Faculty
          </p>
          ...
          {member.role && member.role !== member.title && (
            <p className="text-kawai-charcoal/50 text-sm">{member.role}</p>
          )}

          {member.specialties && (
            <p className="text-kawai-charcoal/55 text-sm mt-6 leading-relaxed max-w-xs">
              {member.specialties}
            </p>
          )}
        </div>
        ...
        {/* Navigation + CTA */}
        <div className="flex items-center justify-between mt-12 pt-6 border-t border-kawai-neutral">
          <div className="flex items-center gap-6">
            <button
              onClick={() => go(index - 1)}
              aria-label="Previous faculty member"
              className="w-10 h-10 flex items-center justify-center text-kawai-charcoal/35 hover:text-kawai-black transition-colors"
            >
            ...
            <span className="text-[12px] font-mono text-kawai-charcoal/30 tracking-widest select-none">
